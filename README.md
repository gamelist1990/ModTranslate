...
`scripts/ModTranslate.ps1` を使うと、同ディレクトリのjarに対してデフォルト `en_us => ja_jp` で実行できます。

- 既定で `--repair-broken-target-in-jar` を有効にしています
- 既定で jar 書き換え前に `<jar>.modtranslate.bak` を作成します（`-NoJarBackup` で無効化）

## GitHub Release（自動ビルド）

GitHub Actions の `release.yml` により、タグを push すると各OS向けのバイナリをビルドして Release に添付します。

- 例: `v0.1.0` を作成して push
- Release には `ModTranslate-<version>-<platform>` がアップロードされます
